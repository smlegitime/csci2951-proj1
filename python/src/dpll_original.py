...
class DPLL:
    ...
    def solve(self, model=None):
        if model is None:
            model = {}
        return self.dpll(self.clauses, self.symbols, model, branch = 0)
    
    def dpll(self, clauses, symbols, model, branch=0):
        """Main DPLL function"""
        ...
        if vars:
            ...
            return self.dpll(clauses, symbols, model, branch)
        ...
        # if (branch < 20): 
        #     p, sign = self.bohm_polarity(symbols, clauses, model)
        # else: 
        #     p, sign = self.branch_var(symbols, clauses, model)
        ...
        res = self.dpll(clauses, rest, (model | {p: sign}), branch + 1)
        if res is not None:
            return res
        
        return self.dpll(clauses, rest, (model | {p: not sign}), branch + 1) # backtracking with False
    ...
    def branch_var(self, symbols, clauses, model):
        """choose based on frequency and also include polarity"""
        var_score = {var: 0 for var in symbols}
    ...
    def bohm_polarity(self, symbols, clauses, model): 
